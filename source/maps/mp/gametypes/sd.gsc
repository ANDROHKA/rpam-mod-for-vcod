/*  rPAMext Version: v21
  ...
  - v211/rCOUNT kikiii fixed when round end and bomb explosion happens at the same time and both in separate threads it could lead to case where player in bomb range supposed to be killed was NOT killed
  - v211/SPAWNLOGICafterTIMEOUT kikiii added a new spawn logic to use the same spawn after a timeout occured
  - rBOMBCOUNTDOWN: retained the old bomb damage of the game (1.1/1.5/pam/rpam)
  - added /set rpam_debug_p 1 debug ingame informations
...
onStartGameType()
{
	...
	{
		//rHINT
		logprint("sd::onStartGameType: 0 attacker spawnpoints found. AbortLevel.\n");
	...
	{
		//rHINT
		logprint("sd::onStartGameType: 0 defender spawnpoints found. AbortLevel.\n");
		maps\mp\gametypes\global\_global::AbortLevel();
		return;
	}
		...
		if (game["is_public_mode"])
		{
			for(;;)
			{
				...
				if (allies > 0 && axis > 0)
				{
					...
					{
						//rHINT
						logprint("sd::onStartGameType Invoked map_restart(true)\n");
						map_restart(true);
					}
...
normalizeDamage(iDamage, health)
{
	...
	{	
		//rHINT
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::normalizeDamage to trim it to 100 hp"); }
		logprint("sd::normalizeDamage onAfterPlayerDamaged dmg is above 100. Trimming it to 100 from: " + iDamage + "\n");
...
markPossibleAssist(eAttacker, normalizedDamage)
{
	...
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::markPossibleAssist " + eAttacker.name + " " + normalizedDamage); }
	logprint("sd::markPossibleAssist " + eAttacker.name + " " + normalizedDamage + "\n");

	if (normalizedDamage >= 41)
	{
		//logprint("_sd::markPossibleAssist 1st if start\n");
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::markPossibleAssist if normalizedDamage >= 41 start"); }
		logprint("sd::markPossibleAssist if normalizedDamage >= 41 start\n");
...
//			logprint("_sd::markPossibleAssist player " + eAttacker.name + " did less damage than " + self.lastAttacker.name + " - " + normalizedDamage + "<" + self.lastAttackerDamage + "\n");
			if (getcvar("rpam_debug_p") == "1") { iprintln("sd::markPossibleAssist player " + eAttacker.name + " did less damage than " + self.lastAttacker.name + " - " + normalizedDamage + "<" + self.lastAttackerDamage); }
			logprint("sd::markPossibleAssist player " + eAttacker.name + " did less damage than " + self.lastAttacker.name + " - " + normalizedDamage + "<" + self.lastAttackerDamage + "\n");
			return;
		}
...
//		logprint("_sd::markPossibleAssist attacker " + self.lastAttacker.name + " is considered for an assit\n");
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::markPossibleAssist attacker " + self.lastAttacker.name + " is considered for an assist"); }
		logprint("sd::markPossibleAssist attacker " + self.lastAttacker.name + " is considered for an assist\n");
	}
}
...
self is the player that was killed.
*/
//rCOUNTkills also while level.roundstarted && level.roundended
...
//		if (!level.in_readyup && level.roundstarted && !level.roundended)
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::Add score to Deaths (not on_readup)"); }
		if (!level.in_readyup)
		{
...
// v211
//		if (!level.in_readyup && level.roundstarted && !level.roundended)
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::Add score to Assists (not on_readup)"); }
		if (!level.in_readyup)
		{
...
//	if (!isDefined(self.switching_teams) && level.roundstarted && !level.roundended)
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::drop weapons after get killed (v211)"); }
	if (!isDefined(self.switching_teams))
...
//		maps\mp\_utility::error("NO " + spawnpointname + " SPAWNPOINTS IN MAP");
	if(isdefined(spawnpoint))
	{
		...
		if (isDefined(game["restoreLastSpawnpoint"]) && game["restoreLastSpawnpoint"])
		{
			...
			{
				logprint("sd::spawnPlayer newSpawnpointLogic 1\n");
				if (getcvar("rpam_debug_p") == "1") { iprintln("sd::spawnPlayer newSpawnpointLogic 1"); }
				self spawn(self.pers["lastSpawnpointOrigin"], self.pers["lastSpawnpointAngles"]);
			}
			else
			{
				logprint("sd::spawnPlayer newSpawnpointLogic 2\n");
				if (getcvar("rpam_debug_p") == "1") { iprintln("sd::spawnPlayer newSpawnpointLogic 2"); }
				self spawn(spawnpoint.origin, spawnpoint.angles);
		...
		{
			logprint("sd::spawnPlayer newSpawnpointLogic 3\n");
			if (getcvar("rpam_debug_p") == "1") { iprintln("sd::spawnPlayer newSpawnpointLogic 3"); }
			self spawn(spawnpoint.origin, spawnpoint.angles);
			self.pers["lastSpawnpointOrigin"] = spawnpoint.origin;
			self.pers["lastSpawnpointAngles"] = spawnpoint.angles;
		}
	...
	{
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::spawnPlayer ERROR SpawnLogic"); }
		maps\mp\_utility::error("NO " + spawnpointname + " SPAWNPOINTS IN MAP");
	}
// --
...
//	logprint("_startRound:: game[restoreLastSpawnpoint]=" + game["restoreLastSpawnpoint"] + "\n");
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::startRound restoreLastSpawnpoint = " + game["restoreLastSpawnpoint"] + ""); }
	logprint("sd::startRound game[restoreLastSpawnpoint]=" + game["restoreLastSpawnpoint"] + "\n");
// --
...
bomb_countdown()
{
	...
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown set level.bombtimer"); }
	countDownTime = level.bombtimer; // seconds
	//PAM
	if (level.show_bombtimer)
	{
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown show hud bombtimer"); }
		level.clock = maps\mp\gametypes\global\_global::newHudElem2();
		level.clock.font = "bigfixed";
		level.clock.alignX = "center";
		level.clock.alignY = "middle";
...
// --
	...
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown count finished"); }
	...
	if(isdefined(level.bombexploder))
		if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown calls _utility::exploder(num)"); }
		maps\mp\_utility::exploder(level.bombexploder);

	logprint("sd::bomb_countdown bomb explode called\n");
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown bomb set damage and range"); }
...
//	mindamage = 1000;
	...
	logprint("sd::bomb_countdown calling delete on defuse trigger and bomb xmodel\n");
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown calling delete bomb model"); }	
	...
	logprint("sd::bomb_countdown deleted bomb timers\n");

	// Add flame around bomb
	playfx(level._effect["bombexplosion"], origin);
	
	logprint("sd::bomb_countdown flame around bomb\n");
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown bomb do damage"); }	
...
//	radiusDamage(origin, range, maxdamage, mindamage);
	...
	logprint("sd::bomb_countdown radius damage completed\n");
	if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_countdown bomb did damage"); }	
	...
	//logprint("sd::bomb_countdown earthquake around bomb\n");


// Display center mission accomplish text to everybody
	if(game["attackers"] == "allies")
	{
		announcement(&"SD_ALLIEDMISSIONACCOMPLISHED");
	}
...
// Play mission accomplish sound on players
	//level thread playSoundOnPlayers("mp_announcer_objdest");

// Count to planting team
...
customRadiusDamage(bombOrigin)
{
	...
	for(i = 0; i < players.size; i++)
	{
        ...
        {
//rHINT
			logprint("sd::customRadiusDamage player " + player.name + " will receive " + selectedDmg + " damage from bomb.\n");
            radiusDamage(player.origin, 0.01, selectedDmg, selectedDmg);
        }
		else
		{
//rHINT
			logprint("sd::customRadiusDamage player=" + player.name + " is too far from bomb. No damamge.\n");
		}
	}
}
...
//v29 rPAM support log
			...
			if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb bomb maxDistance set to 1000"); }
	...
	for(;;)
	{
		...
		if(isPlayer(player) && (player.pers["team"] != level.planting_team) && player isOnGround())
		{
			...
			{	
				...
				{
//v27 added player disableWeapon();
					if (getcvar("rpam_debug_p") == "1") { iprintln("sd::bomb_think player disableWeapon()"); }
					player disableWeapon();
					self.progresstime += level.frame;
					wait level.frame;
				}
